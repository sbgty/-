...
### 1、原始图像的输入与转化
    通过OpenCV读取的图像有着更为精确的数据。
    通过代码实现色彩通道的转化，便于处理数据。

### 2、噪声
    椒盐噪声对于画面的视觉影响整体破坏更加明显，出现明显的不和谐画面。
    高斯噪声对于画面的视觉影响表现体现在色彩的明亮表现。

### 3、滤波
    均值滤波对于椒盐噪声处理能力有限，无法处理明显白黑点。
    中值滤波对于椒盐噪声处理能力明显，
    高斯滤波对于椒盐噪声处理能力有限，在于椒盐噪声多为突变而非连续分布。

## 四、实验小结
本次实验完成了对于彩色图像的读取输出，GRB格式转换为RGB格式的转换处理，灰度图的处理，并研究了噪声的添加方式以及表现形式，最后对于滤波方式进行了实验。
通过本次实验进一步加深了对于图像处理的理解，对于噪声以及滤波的理解。
